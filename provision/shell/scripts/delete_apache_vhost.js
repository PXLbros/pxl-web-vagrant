...
async function main() {
    const selected_apache_site_configuration_file = await choose_apache_site_configuration_file('Which Apache virtual host do you want to delete?');
    ...
    if (await ask_confirm(`Are you sure you want to delete Apache site "${selected_apache_site_configuration_file}"?`)) {
        ...
        const get_server_name_result = exec(`awk '/ServerName/ {print $2}' ${selected_apache_site_configuration_file}`)//, (code, hostname, stderr) => {
        ...
        exec(`sudo rm /etc/apache2/sites-available/${selected_apache_site_configuration_file}`);
