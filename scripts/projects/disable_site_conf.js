...
async function main() {
    ...
    if (options['hostname']) {
        ...
        selected_site_configuration_file = await choose_files_from_dir(web_server_sites_conf_dir, `Which ${web_server_title} site do you want to disable?`);
    }
    ...
    try {
        disable_web_server_site(web_server, selected_site_configuration_file);

        highlight_line(`${web_server_title} site ${selected_site_configuration_file} has been disabled.`);
